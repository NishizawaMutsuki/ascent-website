...
/**
 * GravityCanvas — 重力シミュレーションによるパーティクル＆接続線アニメーション
 *
 * 粒子同士が引き合い、衝突すると合体。一定距離内の粒子間に線を描画。
 * 中央にシールド（反発領域）を設定し、コンテンツとの重なりを防止できる。
 *
 * @example 基本使用（Ascentデフォルト）
 * <GravityCanvas />
 *
 * @example 色・密度をカスタム
 * <GravityCanvas
 ...
 * />
 *
 * @example シールドなし（全面にパーティクル）
 * <GravityCanvas shield={false} />
 *
 * @example 背景として使う
 * <div style={{ position: 'relative', height: '100vh' }}>
 *   <GravityCanvas />
 *   <div style={{ position: 'relative', zIndex: 1 }}>コンテンツ</div>
 * </div>
 */
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      alive.forEach((p) => {
        ...
        ctx.fillStyle = `rgba(${cr},${cg},${cb},${Math.min(0.45, 0.18 + p.r * 0.012)})`;
        ctx.fill();
